...
func JWTAuth(o service.JwtServiceI) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if o.IsInBlacklist(refreshToken) {
		...
		if err != nil {
			// todo 在accessToken中存储updateTime 做到定时刷新 而不是等他过期
			...
			if accessToken == "" || errors.Is(err, utils.ErrTokenExpired) {
				...
				// 如果Refresh Token有效，通过其UserID获取用户信息 todo封装到方法中
