...
import (
	"go_blog/internal/model/appTypes"
...
func ClearRefreshToken(c *gin.Context) {
	...
	//Set-Cookie: m1=hello; Path=/; HttpOnly
	// 调用setCookie设置cookie值为空并过期，删除refresh-token。Cookie: x-refresh-token=""
...
// name	Cookie 名称
// value	Cookie 值
// maxAge	有效期（秒），0 代表会话级 cookie，-1 删除
// path	有效路径，通常 /
// domain	域名或子域名（不写默认为当前域）
// secure	仅 HTTPS 传输时使用
// httpOnly	是否禁止前端 JS 访问
...
func GetClaims(c *gin.Context) (*request.JwtCustomClaims, error) {
	...
	if err != nil {
		// 如果解析失败，记录错误日志
		zap.L().Error("Failed to retrieve JWT parsing information from Gin's Context. Please check if the request header contains 'x-access-token' and if the claims structure is correct.", zap.Error(err))
	}
	return claims, err
}
...
func GetRefreshClaims(c *gin.Context) (*request.JwtCustomRefreshClaims, error) {
	...
	if err != nil {
		// 如果解析失败，记录错误日志
		zap.L().Error("Failed to retrieve JWT parsing information from Gin's Context. Please check if the request header contains 'x-refresh-token' and if the claims structure is correct.", zap.Error(err))
	}
	return claims, err
}
