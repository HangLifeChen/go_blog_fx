...
type JWT struct {
	...
	Issuer               string
	DefaultAccessExpiry  time.Duration
...
func NewJWT() *JWT {
	return &JWT{
		AccessTokenSecret:    []byte("gycyh1106"), // 从全局配置加载 AccessToken 密钥
		RefreshTokenSecret:   []byte("gycyh1106"), // 从全局配置加载 RefreshToken 密钥
		Issuer:               "gycyh1106",
		DefaultAccessExpiry:  time.Hour * 2,
		DefaultRefreshExpiry: time.Hour * 24 * 7,
	}
}

// CreateAccessClaims 创建 Access Token 的 Claims，包含基本信息和过期时间等
func (j *JWT) CreateAccessClaims(baseClaims request.BaseClaims, accessTokenExpiryTime string) request.JwtCustomClaims {
	ep, err := ParseDuration(accessTokenExpiryTime) // 获取过期时间
	if err != nil {
		zap.L().Error("[JWT] ParseDuration error", zap.Error(err))
		ep = j.DefaultAccessExpiry
	}
...
func (j *JWT) CreateRefreshClaims(baseClaims request.BaseClaims, refreshTokenExpiryTime string) request.JwtCustomRefreshClaims {
	ep, err := ParseDuration(refreshTokenExpiryTime) // 获取过期时间 7d2h5m10s
	if err != nil {
		zap.L().Error("[JWT] ParseDuration error", zap.Error(err))
		ep = j.DefaultRefreshExpiry
	}
...
func (j *JWT) ParseAccessToken(tokenString string) (*request.JwtCustomClaims, error) {
	...
	if rc, ok := claims.(*request.JwtCustomClaims); ok {
		if !rc.VerifyAudience("TAP", true) {
			return nil, fmt.Errorf("invalid audience: not TAP")
		}
	}
